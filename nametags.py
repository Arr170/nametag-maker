...
if(namecardMode == 1):
    ...
    for stat in stats:
        img = oneStatCard.make_card(compName, stat['name'], statName1, stat[columnNameStat1], date)
        imgArrey.append(img)
    ...
    for stat in stats:
        img = twoStatCard.make_card(compName, stat['name'], statName1, statName2, stat[columnNameStat1], stat[columnNameStat2])
        imgArrey.append(img)
elif(namecardMode == 3):
    columnNameStat1 = str(input('[a] column name for stat 1: '))
    columnNameStat2 = str(input('[a] column name for stat 2: '))   
    ...
    date = str(input('[a] date of the comp in format dd-dd.mm'))
    for stat in stats:
        img = threeStatCard.make_card(compName, statName1, statName2, statName3, stat[columnNameStat1], stat[columnNameStat2], stat[columnNameStat3], date)
        imgArrey.append(img)
