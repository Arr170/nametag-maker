...
def make_card(compN, csv, template):
    ...
    pdfMaker.generate_big_pdf(img_arr)
...
def main():
    competition_name = input('[?] Enter name of the competition: ')
    source_csv = input('[?] Enter name of source csv file generated by AGE: ')
    options = [
        inquirer.List(
            'tamplate',
            message = 'Choose a tamplate for your nametags',
            choices = ['bg_1.png', 'bg_2.png'],
            default = 'bg_1.png'
        ),
        ]
    answer = inquirer.prompt(options)
    template = answer['tamplate']
    make_card(competition_name, source_csv, template)
